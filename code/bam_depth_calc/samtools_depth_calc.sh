...
if [[ -f "$bam_list" ]]; then
    ...
    ls -1 *.bam > "$out_dir"/temp_bam_list.txt
fi

## Perform depth calculation
samtools depth -Q $mq_min -f "$out_dir"/temp_bam_list.txt |
    awk '{for(i=3;i<=NF;i++) sum+=; print($1,$2-1,$2,sum); sum=0}' |
    awk -v md=$min_dep 'BEGIN {OFS = "\t"} $4 > md' |
    head -n 100000 | # For testing
    bedtools merge -d $max_gap -i stdin > "$out_bed"
